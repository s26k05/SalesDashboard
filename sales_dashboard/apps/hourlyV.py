...
data = pd.read_csv(DATA_PATH.joinpath("saleshourly.csv"))
...
layout_hourly = go.Layout(title = 'Time Series Plot',
                   hovermode = 'closest')
fig = go.Figure(data = trace_1, layout = layout_hourly)

layout = html.Div(
    children=[
        html.Div(
            html.H1(children="HOURLY DATA VISUALIZATION",className="header-title",style={"fontSize": "48px", "color": "black"},),
        ),
        ...
        dcc.Graph(id = 'plot_hourly', figure = fig),
        html.Div(
        children=[
            html.Div(
                children=dcc.Graph(
                    id='price-chart_hourly', config={"displayModeBar": False}
                ),
                className="card",
            ),
...
@app.callback(
    [Output("plot_hourly","figure"),Output("price-chart_hourly","figure")],
    [Input("opt", "value"),
     Input("slider", "value")])
def update_charts(input1, input2):
    ...
    fig = go.Figure(data = [trace_1, trace_2], layout = layout_hourly)
