...
data = pd.read_csv(DATA_PATH.joinpath("salesdaily.csv"))
...
layout_daily = go.Layout(title = 'Time Series Plot',
                   hovermode = 'closest')
fig = go.Figure(data = trace_1, layout = layout_daily)

layout = html.Div(
    children=[
        html.Div(
            html.H1(children="daily data VISUALIZATION",className="header-title",style={"fontSize": "48px", "color": "black"},),
        ),
        ...
        dcc.Graph(id = 'plot_daily', figure = fig),
        html.Div(
        children=[
            html.Div(
                children=dcc.Graph(
                    id='price-chart_daily', config={"displayModeBar": False}
                ),
                className="card",
            ),
...
@app.callback(
    [Output("plot_daily","figure"),Output("price-chart_daily","figure")],
    [Input("opt", "value"),
     Input("slider", "value")])
def update_charts(input1, input2):
    ...
    fig = go.Figure(data = [trace_1, trace_2], layout = layout_daily)
