...
layout_monthly = go.Layout(title = 'Time Series Plot',
                   hovermode = 'closest')
fig = go.Figure(data = trace_1, layout = layout_monthly)

layout = html.Div(

    children=[
        html.Div(
            html.H1(children=" MONTHLY DATA VISUALIZATION",className="header-title",style={"fontSize": "48px", "color": "black"},),
        ),
        ...
        dcc.Graph(id = 'plot_monthly', figure = fig),
        html.Div(
        children=[
            html.Div(
                children=dcc.Graph(
                    id='price-chart_monthly', config={"displayModeBar": False}
                ),
                className="card",
            ),
        ],
...
@app.callback(
    [Output("plot_monthly","figure"),Output("price-chart_monthly","figure")],
    [Input("opt", "value"),
     Input("slider", "value")])
def update_charts(input1, input2):
    ...
    fig = go.Figure(data = [trace_1, trace_2], layout = layout_monthly)
